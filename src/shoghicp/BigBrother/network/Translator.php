		...
		switch($packet->pid()){
			...
			case InboundPacket::USE_ENTITY_PACKET:
				...
				if($frame !== null){
					switch($packet->type){
						case UseEntityPacket::INTERACT:
							...
							$pk->trData = UseItemTransactionData::new([], UseItemTransactionData::ACTION_CLICK_BLOCK, new Position($frame->x, $frame->y, $frame->z, $frame->level), $frame->getFacing(), $player->getInventory()->getHeldItemIndex(), ItemStackWrapper::legacy($player->getInventory()->getItemInHand()), $player->getPosition(), $frame->asVector3(), $frame->getEntityId());
							return $pk;
						case UseEntityPacket::ATTACK:
							if($frame->hasItem()){
								...
								$pk = new InventoryTransactionPacket();
								$pk->trData = UseItemTransactionData::new([], UseItemTransactionData::ACTION_BREAK_BLOCK, new Position($frame->x, $frame->y, $frame->z, $frame->level), $frame->getFacing(), $player->getInventory()->getHeldItemIndex(), ItemStackWrapper::legacy($player->getInventory()->getItemInHand()), $player->getPosition(), $frame->asVector3(), $frame->getEntityId());
							}
							return $pk;
					}
					return null;
				...
				if($packet->type === UseEntityPacket::INTERACT_AT){
					...
					switch($packet->type){
						case UseEntityPacket::INTERACT:
							$type = UseItemOnEntityTransactionData::ACTION_ITEM_INTERACT;
							break;
						case UseEntityPacket::ATTACK:
							$type = UseItemOnEntityTransactionData::ACTION_ATTACK;
							break;
						default:
					...
					$pk->trData = UseItemOnEntityTransactionData::new([], $packet->target, $type, $player->getInventory()->getHeldItemIndex(), ItemStackWrapper::legacy($player->getInventory()->getItemInHand()), $player->getPosition(), new Vector3($packet->targetX, $packet->targetY, $packet->targetZ));
				}
				...
				switch($packet->status){
					case 0:
						if($player->getGamemode() === 1){
							$pk = new InventoryTransactionPacket();
							$pk->trData = UseItemTransactionData::new([], UseItemTransactionData::ACTION_BREAK_BLOCK, new Position($packet->x, $packet->y, $packet->z, $player->getLevelNonNull()), $packet->face, $player->getInventory()->getHeldItemIndex(), ItemStackWrapper::legacy($player->getInventory()->getItemInHand()), $player->getPosition(), new Vector3($packet->x, $packet->y, $packet->z), $player->getLevelNonNull()->getBlockAt($packet->x, $packet->y, $packet->z)->getRuntimeId());
							return $pk;
						}else{
					...
					case 5:
						...
						if($item->getId() === Item::BOW){//Shoot Arrow
							$type = ReleaseItemTransactionData::ACTION_RELEASE;
						}else{//Eating
							$type = ReleaseItemTransactionData::ACTION_CONSUME;
						}

						$pk->trData = ReleaseItemTransactionData::new([], $type, $player->getInventory()->getHeldItemIndex(), ItemStackWrapper::legacy($player->getInventory()->getItemInHand()), new Vector3($packet->x, $packet->y, $packet->z));
			...
			case InboundPacket::PLAYER_BLOCK_PLACEMENT_PACKET:
				...
				$pk = new InventoryTransactionPacket();
				$pk->trData = UseItemTransactionData::new([], UseItemTransactionData::ACTION_CLICK_BLOCK, new Position($packet->x, $packet->y, $packet->z, $player->getLevelNonNull()), $packet->direction, $player->getInventory()->getHeldItemIndex(), ItemStackWrapper::legacy($player->getInventory()->getItemInHand()), $player->getPosition(), new Vector3($packet->x, $packet->y, $packet->z), $player->getLevelNonNull()->getBlockAt($packet->x, $packet->y, $packet->z)->getRuntimeId());
				return $pk;
			...
			case InboundPacket::USE_ITEM_PACKET:
				...
				$pk = new InventoryTransactionPacket();
				$pk->trData = UseItemTransactionData::new([], UseItemTransactionData::ACTION_CLICK_AIR, new Position(0, 0, 0, $player->getLevelNonNull()), -1, $player->getInventory()->getHeldItemIndex(), ItemStackWrapper::legacy($player->getInventory()->getItemInHand()), $player->getPosition(), new Vector3(0, 0, 0), $player->getLevelNonNull()->getBlockAt(0, 0, 0)->getRuntimeId());
				return $pk;
